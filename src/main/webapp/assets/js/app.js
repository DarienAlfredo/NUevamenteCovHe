...
const showConfirmedTotal = (total) => {
  document.querySelector("#confirmed-total").textContent = numberWithCommas(
    total
  );
};

const showRecoveredTotal = (total) => {
  document.querySelector("#recovered-total").textContent = numberWithCommas(
    total
  );
};
...
const renderWorldData = (world_data, status) => {
  let res = [];
...
const loadAllTimeChart = async (country) => {
  let labels = [];
...
const loadDaysChart = async (country) => {
  let labels = [];
